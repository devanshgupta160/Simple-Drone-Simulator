...
if __name__=='__main__':
    ...
    while(1):
        ...
        distance_error['X'].append(waypoints[curr][0]-d.state[0])
        distance_error['Y'].append(waypoints[curr][1]-d.state[1])
        distance_error['Z'].append(waypoints[curr][2]-d.state[2])
        if len(time_array)<=50:
            plt.plot(time_array,distance_error['X'], label = 'Position error along x')
            plt.plot(time_array,distance_error['Y'], label = 'Position error along y')
            plt.plot(time_array,distance_error['Z'], label = 'Position error along z')
        else:
            plt.plot(time_array[-50:],distance_error['X'][-50:], label = 'Position error along x')
            plt.plot(time_array[-50:],distance_error['Y'][-50:], label = 'Position error along y')
            plt.plot(time_array[-50:],distance_error['Z'][-50:], label = 'Position error along z')
        plt.ylabel("Error")
